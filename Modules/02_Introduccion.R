Introduccion <- fluidRow(
  column(
    ...
    tags$ul(
      tags$li(tags$b('(pendiente)'), "Analizar el reporte de", tags$em("Performance Check.")),
      tags$li("Combinar múltiples archivos de intensidades netas (formato Excel y DAC *proximamente*),
              en un solo archivo de Excel.", tags$br(), 
              'Los archivos deben encontrarse comprimidos en un solo archivo ZIP')
    ), 
    tags$br(), tags$br()
  ),
  column(
